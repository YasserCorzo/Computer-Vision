...
for itr in range(max_iters):
    ...
    # divide epoch totals for average accuracy by number of batches to finish calculation
    avg_acc /= batch_num 
    ...
    if 'W' in k:
        for i in range(v.shape[0]):
            for j in range(v.shape[1]):
                # copy the old params
                params_copy = copy.deepcopy(params_orig)
                
                # add eps to current element in weight
                ...
                # compute loss (with eps added)
                loss_add, _ = compute_loss_and_acc(y, probs)
                
                # copy the old params
                params_copy = copy.deepcopy(params_orig)
                
                # subtract eps to current element in weight
                params_copy[k][i, j] -= eps
                ...
                # compute loss (with eps subtracted)
                loss_sub, _ = compute_loss_and_acc(y, probs)
                
                # compute derivative with central diffs
                params["grad_" + k][i, j] = (loss_add - loss_sub) / (2 * eps)
    else:
        for i in range(v.shape[0]):
            # copy the old params
            params_copy = copy.deepcopy(params_orig)
